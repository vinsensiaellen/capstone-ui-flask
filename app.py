...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(by=['Category','App']).agg({"Reviews":"sum", "Rating":'mean'}).sort_values(by='Reviews', ascending=False).head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }

    ## Bar Plot
    cat_order = df2.groupby("Category").agg({
    "App" : "count"}).rename({'App':'Total'}, axis=1).sort_values(by='Total', ascending=False).head()
    X = cat_order.index
    Y = cat_order["Total"]
